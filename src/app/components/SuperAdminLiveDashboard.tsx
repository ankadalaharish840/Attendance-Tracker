...
export default function SuperAdminLiveDashboard({ sessionId, user }: SuperAdminLiveDashboardProps) {
  const [liveData, setLiveData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState("all"); // all, team, agent
  ...
  const { todaySummary, monthSummary, liveStatus, teams } = liveData;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Live Status by Team */}
      <div>
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Live Status by Team</h3>
